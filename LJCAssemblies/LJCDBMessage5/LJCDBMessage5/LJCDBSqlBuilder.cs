  ...
  public class LJCDBSqlBuilder
  {
    ...
    public static string? FilterWhereClause(LJCDBFilters dbFilters, bool recursive = false)
    {
      ...
      if (LJC.HasItems(dbFilters))
      {
        ...
        bool first = true;
        foreach (LJCDBFilter filter in dbFilters)
        {
          ...
          if (LJC.HasItems(conditions))
          {
            ...
            {
              LJCDBCondition condition = conditions[index];
              ...
              if (LJC.IsEqual("is null", condition.SecondValue))
              {
    ...
    public string ColumnList(LJCDBRequest? dbRequest = null, bool listOnly = true)
    {
      ...
      else
      {
        ...
        {
          foreach (LJCDBJoin join in dbRequest.Joins)
          {
    ...
    private string GetJoinOns(LJCDBJoin dbJoin, LJCDBJoinOns dbJoinOns
    ...
    private string GetJoinTableString(string? schemaName, LJCDBJoin dbJoin)
    {
      string retValue;
    ...
    private readonly LJCDBRequest mDbRequest;
  }
}
