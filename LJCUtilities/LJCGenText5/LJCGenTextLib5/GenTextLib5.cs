...
namespace LJCGenTextLib5
{
  ...
  ///  path='items/GenTextLib/*'/>
  public class GenTextLib
  {
    #region Properties
    ...
    public string TextGen(Sections sections, string[] templateLines)
    {
      ...
      for (var lineIndex = 0; lineIndex < templateLines.Length; lineIndex++)
      {
        ...
        if (directive != null)
        {
          ...
          if (!LJC.IsEqual("#sectionbegin", name))
          {
            continue;
          }
        }
      ...
      , ref int lineIndex)
    {
      ...
      if (success)
      {
        ...
        if (LJC.HasText(value))
        {
          if (LJC.IsEqual("hasvalue", directive.Value)
      ...
      , ref int nextLineIndex)
    {
      ...
      if (LJC.IsEqual("subsection", directive.Name)
    ...
    private static bool IsInValues(string text, params string[] values)
    {
      bool retValue = false;
    ...
    private static bool IsNotProcessValues(string name)
    {
    ...
    private static bool IsProcessValues(string name)
    {
    ...
    private bool IsConfig(string line, out Directive? directive)
    {
      ...
      if (line.Contains("#commentchars", StringComparison.OrdinalIgnoreCase))
      {
        string[]? values = LJCNetString.Split(line, " ");
        if (values?.Length >= 2)
        {
