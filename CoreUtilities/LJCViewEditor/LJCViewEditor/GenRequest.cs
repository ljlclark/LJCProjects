...
namespace LJCViewEditor
{
  ...
  public class GenRequest
  {
    ...
    public static string RequestCode(DbRequest dbRequest)
    {
      ...
      {
        "0   DbRequest dbRequest = new DbRequest(RequestType.Load\")",
        "1   \", \"{0}\", \"{1}\")",
        "2   {{",
        "3   {0}",
        "4   }}",
      ...
      builder.Append(Line(request, 0));
      builder.Append(Line(request, 1, req.TableName, req.DataConfigName));
      builder.AppendLine(Line(request, 2));
      var code = ColumnsCode(req.Columns, indent);
      builder.Append(Line(request, 3, code));
      builder.AppendLine(Line(request, 4));
      code = JoinsCode(req, indent);
      builder.Append(Line(request, 3, code));
      code = FiltersCode(req, indent);
      builder.Append(Line(request, 3, code));
      code = OrderByNamesCode(req.OrderByNames, indent);
      builder.Append(Line(request, 3, code));

      //builder.Append("  DbRequest dbRequest = new DbRequest(RequestType.Load");
      //builder.AppendLine($", \"{req.TableName}\", \"{req.DataConfigName}\")");
      //builder.AppendLine("  {");
      //builder.Append(ColumnsCode(req.Columns, indent));
      //builder.AppendLine("  };");
      //builder.Append(JoinsCode(req, indent));
      //builder.Append(FiltersCode(req, indent));
      //builder.Append(OrderByNamesCode(req.OrderByNames, indent));
