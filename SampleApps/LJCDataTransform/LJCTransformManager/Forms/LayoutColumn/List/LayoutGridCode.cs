...
{
	// Code for the Layout Grid control.
	internal class LayoutGridCode
	{
		#region Constructors

		// Initializes an object instance.
		internal LayoutGridCode(LayoutColumnList parent)
		{
			mParent = parent;
		...
		internal void DataRetrieveLayout()
		{
			...
			if (NetCommon.HasItems(records))
			{
				...
				if (mParent.LJCSelectedRecord != null)
				{
					RowSelectLayout(mParent.LJCSelectedRecord);
					mParent.LJCSelectedRecord = null;
				}
			}
			mParent.Cursor = Cursors.Default;
			mParent.DoChange(LayoutColumnList.Change.Layout);
		}
		...
		private void SetStoredValuesLayout(LJCGridRow row
			, SourceLayout dataRecord)
		{
		...
		private void LayoutDetail_LJCChange(object sender, EventArgs e)
		{
			...
			else
			{
				...
				mLayoutGrid.LJCSetCurrentRow(row, true);
				mParent.TimedChange(LayoutColumnList.Change.Layout);
			}
		}
		...
		internal void DoSelectLayout()
		{
			...
			if (mLayoutGrid.CurrentRow is LJCGridRow row)
			{
				...
				var sourceLayoutManager = mManagers.SourceLayoutManager;
				var keyColumns = sourceLayoutManager.GetIDKey(id);
				record = sourceLayoutManager.Retrieve(keyColumns);
				if (record != null)
				{
		...
		private readonly LayoutColumnList mParent;
