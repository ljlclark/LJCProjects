...
namespace LJCDataDetailConsole
{
	...
	public class CitySectionItem
	{
		...
		public CitySectionItem(string dataConfigName)
		{
		...
		/// </summary>
		/// <param name="parentRecordColumns">The Parent RecordColumns.</param>
		/// <param name="propertyName">The Parent PropertyName.</param>
		/// <param name="staticKey">True if a StaticKey, otherwise false.</param>
		/// <returns>The KeyItem object.</returns>
		public KeyItem KeyItem(DbColumns parentRecordColumns, string propertyName
			, bool staticKey = false)
		{
			KeyItem retValue = null;

			int id = parentRecordColumns.LJCGetInt(Province.ColumnID);
			if (id > 0)
			{
				var manager = mRegionManagers.CitySectionManager;
